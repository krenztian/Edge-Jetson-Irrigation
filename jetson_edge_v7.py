...
def load_latest_sensor_data_from_influxdb():
    """
    On startup, fetch missing cycles from InfluxDB and populate local_15min_records.

    Recovery Logic:
    1. Get the last cycle stored locally (if any)
    2. Get the latest cycle from InfluxDB
    3. Fetch all missing cycles between them
    4. Add missing cycles to local_15min_records
    5. Update Live Sensor Readings with the latest data
    """
    global live_sensor_metadata, local_15min_records

    if not INFLUXDB_AVAILABLE:
        logger.warning("InfluxDB not available - cannot load latest sensor data")
        return False

    # Get latest cycle info from InfluxDB
    latest = influx_recovery.query_latest_cycle_info()

    if not latest:
        logger.info("No recent sensor data found in InfluxDB")
        return False

    influx_cycle = latest.get("cycle")
    influx_date = latest.get("cycle_date")

    logger.info(f"InfluxDB latest: Cycle {influx_cycle}/96 for {influx_date}")

    # Get last local cycle (if any)
    local_cycle = None
    local_date = None
    if local_15min_records:
        last_local = local_15min_records[-1]
        local_cycle = last_local.get("cycle")
        local_date = last_local.get("cycle_date")
        logger.info(f"Local latest: Cycle {local_cycle}/96 for {local_date}")
    else:
        logger.info("No local records found - will fetch from InfluxDB")

    # Determine what cycles to fetch
    cycles_to_fetch = []

    if influx_cycle and influx_date:
        if local_cycle is None or local_date is None:
            # No local data - fetch all cycles for today from InfluxDB
            logger.info(f"Fetching all cycles for {influx_date} from InfluxDB...")
            influx_records = influx_recovery.query_cycles_for_date(influx_date)

            for rec in influx_records:
        ...
        elif local_date == influx_date and local_cycle < influx_cycle:
            # Same day, missing cycles between local and influx
            logger.info(f"Fetching missing cycles {local_cycle + 1} to {influx_cycle} for {influx_date}...")
            influx_records = influx_recovery.query_cycles_for_date(influx_date)

            for rec in influx_records:
                rec_cycle = rec.get("cycle")
                if rec_cycle and local_cycle < rec_cycle <= influx_cycle:
                    # Check for duplicates
                    is_dup = False
                    for existing in local_15min_records:
                        if existing.get("cycle") == rec_cycle and existing.get("cycle_date") == influx_date:
                            is_dup = True
                            break
                    if not is_dup:
                        local_15min_records.append(rec)
                        logger.info(f"Recovered missing cycle {rec_cycle}/96 for {influx_date}")

        elif local_date != influx_date:
            # Different day - fetch all of today's cycles
            logger.info(f"Date changed ({local_date} -> {influx_date}), fetching all cycles...")
            influx_records = influx_recovery.query_cycles_for_date(influx_date)
    ...
    try:
        if 'T' in str(original_ts):
            dt = datetime.fromisoformat(str(original_ts).replace('Z', '').split('+')[0])
        else:
            dt = datetime.fromisoformat(original_ts)
    ...
    logger.info(f"Startup recovery complete. Local records: {len(local_15min_records)}, Latest: Cycle {influx_cycle}/96")
    return True
...
class InfluxDBRecovery:
    ...
    def query_cycles_for_date(self, cycle_date: str) -> list:
        """
        Query all cycles for a specific date from InfluxDB.
        Returns list of records with cycle_number field.

        Args:
            cycle_date: Date string in YYYY-MM-DD format
        """
        if not self.client:
            return []

        try:
            # Query records that have cycle_date tag matching
            # Since day starts at 06:00, we need to check a 24-hour window
            query = f'''
            from(bucket: "{INFLUXDB_CONFIG['bucket']}")
                |> range(start: -48h)
                |> filter(fn: (r) => r["_measurement"] == "weather_15min")
                |> filter(fn: (r) => r["cycle_date"] == "{cycle_date}")
                |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")
                |> sort(columns: ["_time"], desc: false)
            '''

            logger.info(f"Querying InfluxDB for cycles on {cycle_date}...")
            tables = self.query_api.query(query, org=INFLUXDB_CONFIG["org"])
            ...
            logger.error(f"Failed to query cycles for {cycle_date}: {e}")
            return []

    def query_latest_cycle_info(self) -> dict:
        """
        Get the latest cycle number and date from InfluxDB.
        Returns dict with cycle, cycle_date, and full record.
        """
        if not self.client:
            return None
        ...
        try:
            ...
            for table in tables:
                for record in table.records:
                    return {
                        "cycle": record.values.get("cycle_number"),
                        "cycle_date": record.values.get("cycle_date"),
                        "timestamp": record.get_time().isoformat(),
                        "temp_min_15": record.values.get("temp_min_15"),
                        "temp_max_15": record.values.get("temp_max_15"),
                        "temp_avg_15": record.values.get("temp_avg_15"),
            ...
            logger.error(f"Failed to query latest cycle info: {e}")
            return None
...
def store_15min_record_locally(data: dict):
    ...
    Called when ESP32 sends data to /receive-15min
    All float values rounded to 2 decimals

    New V5 format includes:
    - cycle: Cycle number 1-96 (day starts at 06:00)
    - total_cycles: Always 96
    - cycle_date: YYYY-MM-DD (the day this cycle belongs to)
    """
    tmin = data.get("tmin")
    tmax = data.get("tmax")
    ...
    record = {
        "cycle": data.get("cycle"),                    # Cycle number 1-96
        "total_cycles": data.get("total_cycles", 96),  # Always 96
        "cycle_date": data.get("cycle_date"),          # YYYY-MM-DD
    ...
    # Check for duplicate cycle (same cycle_date and cycle number)
    cycle_num = data.get("cycle")
    cycle_date = data.get("cycle_date")
    if cycle_num and cycle_date:
        for existing in local_15min_records:
            if existing.get("cycle") == cycle_num and existing.get("cycle_date") == cycle_date:
                logger.info(f"Duplicate cycle {cycle_num}/{cycle_date} - skipping")
                return

    local_15min_records.append(record)
    logger.info(f"Stored cycle {cycle_num}/96 for {cycle_date} (total records: {len(local_15min_records)})")
...
async def receive_15min_data(data: dict):
    ...
    V5 Payload format (with fixed cycle timing):
    {
        "cycle": 5,                    // Cycle number 1-96
        "total_cycles": 96,            // Always 96
        "cycle_date": "2026-01-31",    // Day this cycle belongs to (starts at 06:00)
        "timestamp": "2026-01-31T07:15:00",
        "tmin": 29.0,
        "tmax": 29.5,
        ...
    }
    """
    try:
        # Extract cycle info from ESP32 payload
        cycle_num = data.get("cycle")
        cycle_date = data.get("cycle_date")
        ...
        record = {
            ...
            "total_cycles": total_cycles,          # Always 96
        ...
        # Check for duplicate by cycle number and date (more reliable than timestamp)
        is_duplicate = False
        if cycle_num and cycle_date:
            for existing in local_15min_records:
                if existing.get("cycle") == cycle_num and existing.get("cycle_date") == cycle_date:
                    is_duplicate = True
                    logger.debug(f"Skipping duplicate cycle {cycle_num}/{cycle_date}")
                    break

        # Store locally only if not duplicate
        if not is_duplicate:
            local_15min_records.append(record)
            logger.info(f"Stored cycle {cycle_num}/96 for {cycle_date}")
        else:
            logger.info(f"Duplicate cycle {cycle_num}/{cycle_date} skipped")
...
async def dashboard():
    ...
    for record in recent_records:
        ...
        if timestamp:
            try:
                # Handle different timestamp formats
                # ESP32 already sends Thailand time, so NO conversion needed
    ...
    # Live Sensor Readings - use latest record from local_15min_records (same as Raw Data tab)
    ...
    if local_15min_records:
        ...
        current_temp_min = round(latest_record.get("temp_min_15") or 0, 2)
        current_temp_max = round(latest_record.get("temp_max_15") or 0, 2)
        current_humidity = round(latest_record.get("rh_avg_15") or 0, 2)
        current_wind = round(latest_record.get("wind_avg_15") or 0, 2)
        current_sunshine_hours = round(latest_record.get("sunshine_hours_15") or 0, 4)
        current_pressure = round(latest_record.get("pressure_avg_15") or 0, 2)
        current_vpd = round(latest_record.get("vpd_avg_15") or 0, 2)
        current_solar_wm2 = round(latest_record.get("solar_avg_15") or 0, 2)
        current_rainfall = round(latest_record.get("rain_mm_15") or 0, 2)

        # Get timestamp - ESP32 already sends Thailand time, no conversion needed
        ...
        # Determine source: live or restored
    ...
    # Pre-compute sensor status classes for HTML
    # online = live data from ESP32, standby = restored data from InfluxDB, offline = no data/error
    def get_sensor_status_class(sensor_name):
        sensor_online = sensor_status.get(sensor_name, {}).get('online')
        if sensor_online:
            if live_data_source == "restored":
                return 'standby'
            return 'online'
        return 'offline'

    temp_status_class = get_sensor_status_class('temperature')
    humidity_status_class = get_sensor_status_class('humidity')
    wind_status_class = get_sensor_status_class('wind')
    solar_status_class = get_sensor_status_class('solar')
    rain_status_class = get_sensor_status_class('rain')
    pressure_status_class = get_sensor_status_class('pressure')
    sunshine_status_class = get_sensor_status_class('sunshine')

    # Status text labels (English / Thai)
    status_text = {
        'online': {'en': 'Online', 'th': 'ออนไลน์'},
        'standby': {'en': 'Standby', 'th': 'สแตนด์บาย'},
    ...
    <head>
        ...
        <style>
            :root {{
                ...
                --accent: #2E7D32;          /* Forest Green - Success/Growth */
                --accent-light: #E8F5E9;
                --highlight: #FF6B35;       /* Coral Orange - Alerts/CTAs */
                --highlight-light: #FFF0EB;
                --neutral: #546E7A;         /* Steel Gray - Secondary text */
            ...
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .datetime-date {{ font-size: 0.85rem; font-weight: 600; color: var(--primary); }}
            .datetime-time {{ font-size: 0.7rem; color: var(--gray-500); }}
            ...
            .section-icon.orange {{ background: var(--highlight-light); color: var(--highlight); }}
            .section-title {{ font-size: 1rem; font-weight: 600; color: var(--primary); }}
            ...
            .chart-title svg {{ width: 16px; height: 16px; color: var(--accent); }}
            .chart-badge {{
                font-size: 0.65rem;
                padding: 3px 10px;
                border-radius: 12px;
    ...
    <body>
            ...
            <aside class="sidebar">
                ...
                <nav class="sidebar-nav">
                    <a href="/dashboard" class="nav-item active">
                        ...
                        <span data-en="Dashboard" data-th="แดชบอร์ด">Dashboard</span>
                    </a>
                    <a href="/config" class="nav-item">
                        <span class="nav-icon"><i data-lucide="settings"></i></span>
                        <span data-en="Settings" data-th="การตั้งค่า">Settings</span>
                    </a>
                    <a href="/raw-data" class="nav-item">
                        <span class="nav-icon"><i data-lucide="database"></i></span>
                        <span data-en="Raw Data" data-th="ข้อมูลดิบ">Raw Data</span>
                    </a>
                    <a href="/docs" class="nav-item">
                        <span class="nav-icon"><i data-lucide="file-text"></i></span>
                        <span data-en="API Docs" data-th="เอกสาร API">API Docs</span>
                    </a>
                </nav>
                <div class="farm-info">
                    <div class="farm-name"><i data-lucide="trees"></i> {irrigation_config.get('farm_name', 'My Farm')}</div>
                    <div class="farm-stats">
                        <div class="farm-stat">
                            <div class="farm-stat-value">{current_day}</div>
                            <div class="farm-stat-label" data-en="Day" data-th="วัน">Day</div>
                        </div>
                        <div class="farm-stat">
                            <div class="farm-stat-value">{current_kc:.2f}</div>
                            <div class="farm-stat-label" data-en="Kc" data-th="Kc">Kc</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="sidebar-climate">
                    <div class="sidebar-climate-title">
                        <i data-lucide="brain-circuit"></i>
                        <span data-en="AI Climate" data-th="AI สภาพอากาศ">AI Climate</span>
                    </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; background: rgba(255,255,255,0.1); border-radius: 6px;">
                            <span style="font-size: 0.7rem; color: rgba(255,255,255,0.7);">ETo</span>
                            <span style="font-size: 0.85rem; font-weight: 600; color: var(--white);">{current_eto:.2f} <small style="opacity:0.6">mm</small></span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; background: rgba(46,125,50,0.3); border-radius: 6px; border-left: 3px solid var(--accent);">
                            <span style="font-size: 0.7rem; color: rgba(255,255,255,0.7);">ETc</span>
                            <span style="font-size: 0.85rem; font-weight: 600; color: #81C784;">{current_etc:.2f} <small style="opacity:0.6">mm</small></span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; background: rgba(255,107,53,0.2); border-radius: 6px;">
                            <span style="font-size: 0.7rem; color: rgba(255,255,255,0.7);">Rn</span>
                            <span style="font-size: 0.85rem; font-weight: 600; color: #FFB74D;">{current_rn_est:.2f} <small style="opacity:0.6">MJ</small></span>
                        </div>
                    </div>
                </div>

                <!-- Location Display (subtle) -->
                <div style="margin: 16px 12px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 6px;">
                    <div style="display: flex; align-items: center; gap: 6px; font-size: 0.65rem; color: rgba(255,255,255,0.5);">
                        <i data-lucide="map-pin" style="width: 12px; height: 12px;"></i>
                        <span id="location-text" data-en="Detecting location..." data-th="กำลังหาตำแหน่ง...">Detecting location...</span>
                    </div>
                </div>
            </aside>
            ...
            <main class="main-content">
                ...
                <div class="top-bar">
                    <div class="top-bar-left">
                        ...
                        <div class="location-display" id="location-display-main" style="display: none;">
                            <i data-lucide="map-pin"></i>
                            <span class="location-text" id="location-main-text"></span>
                        </div>
                    </div>
                    <div class="top-bar-right">
                        <!-- Font Size Controls -->
                        <div class="font-controls">
                            <button class="font-btn" onclick="changeFontSize(-0.1)" title="Decrease font size">
                                <i data-lucide="minus"></i>
                            </button>
                            <button class="font-btn" onclick="changeFontSize(0)" title="Reset font size">
                                <i data-lucide="type"></i>
                            </button>
                            <button class="font-btn" onclick="changeFontSize(0.1)" title="Increase font size">
                                <i data-lucide="plus"></i>
                            </button>
                        </div>
                        <div class="datetime-display">
                            <div class="datetime-date">{datetime.now().strftime("%a, %d %b %Y")}</div>
                            <div class="datetime-time">{datetime.now().strftime("%H:%M")} • <span data-en="Auto 30s" data-th="อัตโนมัติ 30วิ">Auto 30s</span></div>
                        </div>
                        <div class="lang-toggle">
                            <button class="lang-btn active" onclick="setLanguage('en')" id="lang-en">
                                <img src="https://flagcdn.com/w20/gb.png" alt="EN" class="lang-flag">
                ...
                <div class="sensors-section">
                    <div class="section-header">
                        <div class="section-icon blue"><i data-lucide="radio-tower"></i></div>
                        <span class="section-title">
                            ...
                            <span style="font-size: 0.7rem; font-weight: normal; margin-left: 8px;">
                                {f'<span style="color: var(--accent);">● <span data-en="Live" data-th="สด">Live</span></span>' if live_data_source == "live" else f'<span style="color: var(--highlight);">● <span data-en="Restored" data-th="กู้คืน">Restored</span></span>' if live_data_source == "restored" else '<span style="color: var(--neutral);">● <span data-en="Waiting" data-th="รอข้อมูล">Waiting</span></span>'}
                                <span style="color: var(--neutral); margin-left: 4px;"><span data-en="Collected" data-th="เก็บข้อมูลเมื่อ">Collected</span>: {live_data_timestamp}</span>
                            </span>
                        </span>
                    </div>
                    <div class="sensor-grid">
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--primary-light); color: var(--primary);"><i data-lucide="thermometer"></i></div>
                                <div class="sensor-status {temp_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[temp_status_class]['en']}" data-th="{status_text[temp_status_class]['th']}">{status_text[temp_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Temperature" data-th="อุณหภูมิ">Temperature</div>
                            <div class="sensor-value">{current_temp_max:.2f}<span class="sensor-unit">°C</span></div>
                            <div style="font-size: 0.7rem; color: var(--neutral);"><span data-en="Max" data-th="สูงสุด">Max</span> | <span data-en="Min" data-th="ต่ำสุด">Min</span>: {current_temp_min:.2f}°C</div>
                        ...
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--primary-light); color: var(--primary);"><i data-lucide="droplets"></i></div>
                                <div class="sensor-status {humidity_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[humidity_status_class]['en']}" data-th="{status_text[humidity_status_class]['th']}">{status_text[humidity_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Humidity" data-th="ความชื้น">Humidity</div>
                            <div class="sensor-value">{current_humidity:.2f}<span class="sensor-unit">%</span></div>
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {current_humidity}%; background: var(--primary);"></div></div>
                        </div>
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--accent-light); color: var(--accent);"><i data-lucide="wind"></i></div>
                                <div class="sensor-status {wind_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[wind_status_class]['en']}" data-th="{status_text[wind_status_class]['th']}">{status_text[wind_status_class]['en']}</span></div>
                            </div>
                            ...
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {min(current_wind*10, 100)}%; background: var(--accent);"></div></div>
                        </div>
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--highlight-light); color: var(--highlight);"><i data-lucide="sun"></i></div>
                                <div class="sensor-status {solar_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[solar_status_class]['en']}" data-th="{status_text[solar_status_class]['th']}">{status_text[solar_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Solar Radiation" data-th="รังสีดวงอาทิตย์">Solar Radiation</div>
                            <div class="sensor-value">{current_solar_wm2:.2f}<span class="sensor-unit">W/m²</span></div>
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {min(current_solar_wm2/10, 100)}%; background: #ffa726;"></div></div>
                        </div>
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--primary-light); color: var(--primary);"><i data-lucide="cloud-rain"></i></div>
                                <div class="sensor-status {rain_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[rain_status_class]['en']}" data-th="{status_text[rain_status_class]['th']}">{status_text[rain_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Rainfall" data-th="ปริมาณฝน">Rainfall</div>
                            <div class="sensor-value">{current_rainfall:.2f}<span class="sensor-unit">mm</span></div>
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {min(current_rainfall*5, 100)}%; background: var(--primary);"></div></div>
                        </div>
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--neutral-light); color: var(--neutral);"><i data-lucide="gauge"></i></div>
                                <div class="sensor-status {pressure_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[pressure_status_class]['en']}" data-th="{status_text[pressure_status_class]['th']}">{status_text[pressure_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Pressure" data-th="ความกดอากาศ">Pressure</div>
                            <div class="sensor-value">{current_pressure:.2f}<span class="sensor-unit">hPa</span></div>
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {min((current_pressure-950)/100*100, 100) if current_pressure else 0}%; background: var(--neutral);"></div></div>
                        </div>
                        <div class="sensor-card">
                            <div class="sensor-header">
                                <div class="sensor-icon" style="background: var(--highlight-light); color: var(--highlight);"><i data-lucide="clock"></i></div>
                                <div class="sensor-status {sunshine_status_class}"><div class="sensor-status-dot"></div><span data-en="{status_text[sunshine_status_class]['en']}" data-th="{status_text[sunshine_status_class]['th']}">{status_text[sunshine_status_class]['en']}</span></div>
                            </div>
                            <div class="sensor-name" data-en="Sunshine" data-th="แสงแดด">Sunshine</div>
                            <div class="sensor-value">{current_sunshine_hours:.4f}<span class="sensor-unit">hrs</span></div>
                            <div style="font-size: 0.7rem; color: var(--neutral);" data-en="Last 15-min period (max 0.25h)" data-th="ช่วง 15 นาทีล่าสุด (สูงสุด 0.25 ชม.)">Last 15-min period (max 0.25h)</div>
                            <div class="sensor-bar"><div class="sensor-bar-fill" style="width: {min(current_sunshine_hours/0.25*100, 100)}%; background: var(--highlight);"></div></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="daily-section">
                    <div class="section-header">
                        <div class="section-icon green"><i data-lucide="brain-circuit"></i></div>
                        <span class="section-title" data-en="AI Climate Analysis" data-th="วิเคราะห์สภาพอากาศ AI">AI Climate Analysis</span>
                        <span style="margin-left: auto; font-size: 0.75rem; color: var(--neutral);" data-en="Last 10 Days" data-th="10 วันล่าสุด">Last 10 Days</span>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 16px;">
                        ...
                        <div class="chart-card">
                            <div class="chart-header">
                                <span class="chart-title"><i data-lucide="sprout"></i> <span data-en="Daily Plant Water Loss" data-th="การสูญเสียน้ำพืชรายวัน">Daily Plant Water Loss</span></span>
                                <div style="display: flex; gap: 12px; font-size: 0.7rem;">
                                    <span style="display: flex; align-items: center; gap: 4px;">
                                        <span style="width: 12px; height: 3px; background: var(--primary); border-radius: 2px;"></span>
                                        ETo
                                    </span>
                                    <span style="display: flex; align-items: center; gap: 4px;">
        ...
        <script>
            ...
            function changeFontSize(delta) {{
                ...
                let currentScale = parseFloat(getComputedStyle(root).getPropertyValue('--font-scale')) || 1;

                if (delta === 0) {{
                    // Reset to default
                    currentScale = 1;
                }} else {{
                    currentScale = Math.max(0.8, Math.min(1.4, currentScale + delta));
                }}

                root.style.setProperty('--font-scale', currentScale);
                localStorage.setItem('aquasense_font_scale', currentScale);
            }}

            // Location Detection Function
            function detectLocation() {{
                const locationText = document.getElementById('location-text');
                const locationMainText = document.getElementById('location-main-text');
                const locationDisplay = document.getElementById('location-display-main');
                        ...
                        async (position) => {{
                            ...
                            try {{
                                ...
                                if (data.address) {{
                                    ...
                                    locationName = addr.village || addr.town || addr.city || addr.county || addr.state || '';
                                ...
                                if (locationMainText) {{
                                    locationMainText.textContent = locationName;
                                    locationDisplay.style.display = 'flex';
                                }}

                                // Save to localStorage
                                localStorage.setItem('aquasense_location', locationName);
                            }} catch (error) {{
                                const fallback = `${{lat.toFixed(4)}}°, ${{lon.toFixed(4)}}°`;
                                if (locationText) locationText.textContent = fallback;
                            }}
                        }},
                        (error) => {{
                            // Try to load from localStorage
                            const savedLocation = localStorage.getItem('aquasense_location');
            ...
            document.addEventListener('DOMContentLoaded', function() {{
                ...
                const savedFontScale = localStorage.getItem('aquasense_font_scale');
                if (savedFontScale) {{
...
if __name__ == "__main__":
    uvicorn.run("jetson_edge_v7:app", host="0.0.0.0", port=8000, log_level="info", reload=False)
